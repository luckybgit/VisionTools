...
local Type, Version = "VTPullButton", 1
...
local function GetDropTarget()
  ...
  local buttonList = VT.main_frame.sidePanel.newPullButtons
  ...
  if scrollFrame.frame:IsMouseOver(1, -1, -dragdrop_overlap, dragdrop_overlap) then
    ...
    if not id then
      local addNewPullButton = VT.main_frame.sidePanel.newPullButton
      if addNewPullButton.frame:IsMouseOver(2) then
        local maxPulls = #VT:GetCurrentPreset().value.pulls
        id = maxPulls
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      MenuUtil.CreateContextMenu(VT.main_frame, function(ownerRegion, rootDescription)
        if buttonSelf.index ~= 1 then
          rootDescription:CreateButton(L["Pull Drop Move up"], function()
            VT:MovePullUp(buttonSelf.index)
        ...
        if buttonSelf.index < buttonSelf.maxPulls then
          rootDescription:CreateButton(L["Pull Drop Move down"], function()
            VT:MovePullDown(buttonSelf.index)
            if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
              VT:LiveSession_SendPulls(VT:GetPulls())
            end
          end)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert before"], function()
          VT:PresetsAddPull(self.index)
          VT:ReloadPullButtons()
          VT:SetSelectionToPull(self.index)
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        end)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert after"], function()
          VT:PresetsAddPull(buttonSelf.index + 1)
          VT:ReloadPullButtons()
          VT:SetSelectionToPull(buttonSelf.index + 1)
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        end)
        ...
        if buttonSelf.index ~= 1 then
          rootDescription:CreateButton(L["Pull Drop Merge up"], function()
            local newIndex = VT:PresetsMergePulls(buttonSelf.index, buttonSelf.index - 1)
            VT:ReloadPullButtons()
            VT:SetSelectionToPull(newIndex)
            if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
              VT:LiveSession_SendPulls(VT:GetPulls())
            end
          end)
    ...
    local function openMultiContextMenu()
      MenuUtil.CreateContextMenu(VT.main_frame, function(ownerRegion, rootDescription)
        rootDescription:CreateButton(L["Pull Drop Insert before"], function()
          ...
          VT:ReloadPullButtons()
          VT:SetSelectionToPull(self.index)
          --VT:UpdateAutomaticColors(self.index)
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        end)
        ...
        rootDescription:CreateButton(L["Pull Drop Insert after"], function()
          ...
          VT:PresetsAddPull(self.index + 1)
          VT:ReloadPullButtons()
          VT:SetSelectionToPull(self.index + 1)
          --VT:UpdateAutomaticColors(self.index + 1)
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        end)
        ...
        rootDescription:CreateButton(L["Pull Drop Merge"], function()
          local selected_pulls = VT.U.copy(VT:GetSelection())
          -- Assure, that the destination is always the last selected_pull, to copy it's options at last
          VT.U.iremove_if(selected_pulls, function(pullIdx)
            return pullIdx == self.index
          end)

          if not VT.U.contains(selected_pulls, self.index) then
            tinsert(selected_pulls, self.index)
          end

          local newIndex = VT:PresetsMergePulls(selected_pulls, self.index)
          VT:ReloadPullButtons()
          VT:GetCurrentPreset().value.selection = { newIndex }
          VT:SetSelectionToPull(newIndex)
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        ...
        rootDescription:CreateButton(L["Pull Drop Clear Pulls"], function()
          if not VT.U.contains(VT:GetSelection(), self.index) then
            tinsert(VT:GetSelection(), self.index)
            self:Pick()
          end

          for _, pullIdx in ipairs(VT:GetSelection()) do
            VT:ClearPull(pullIdx)
          end
          if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
            VT:LiveSession_SendPulls(VT:GetPulls())
          end
        ...
        if self.maxPulls > 1 then
          rootDescription:CreateButton(L["Pull Drop Delete"], function()
            local addPull = false
            local button = VT:GetFirstNotSelectedPullButton(self.index, "UP")
            if not button then
              button = VT:GetFirstNotSelectedPullButton(self.index, "DOWN")
              if not button then
                addPull = true
                button = 1
              end
            ...
            local removed_pulls = {}
            for _, pullIdx in pairs(VT.GetSelection()) do
              local offset = VT.U.count_if(removed_pulls, function(entry)
                return entry < pullIdx
              end)

              VT:DeletePull(pullIdx - offset)
              tinsert(removed_pulls, pullIdx)
            end

            VT.GetCurrentPreset().value.selection = {}

            if not addPull then
              local offset = VT.U.count_if(removed_pulls, function(entry)
                return entry < button
              end)
              VT:SetSelectionToPull(button - offset)
            else
              --VT:AddPull(1) --we handle not deleting all pulls in VT:DeletePull() instead
              VT:SetSelectionToPull(1)
            end
            VT:ReloadPullButtons()
            if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
              VT:LiveSession_SendPulls(VT:GetPulls())
    ...
    function self.callbacks.OnClickNormal(_, mouseButton, force)
      if not force and not MouseIsOver(VT.main_frame.sidePanel.pullButtonsScrollFrame.frame) then return end
      ...
      if (IsControlKeyDown()) then
        if (mouseButton == "LeftButton") then
          ...
          if not VT.U.contains(VT:GetSelection(), self.index) then
            tinsert(VT:GetSelection(), self.index)
            local changed = VT:SetMapSublevel(self.index)
            VT:SetSelectionToPull(VT:GetSelection())
            if changed then VT:UpdateMap() end
          else
            VT.U.iremove_if(VT:GetSelection(), function(entry)
              return entry == self.index
            end)
            self:ClearPick()
          end
      ...
      elseif (IsShiftKeyDown()) then
        if (mouseButton == "LeftButton") then
          ...
          for i = lastPull, self.index, step do
            if not VT.U.contains(selection, i) then
              tinsert(selection, i)
            end
          end
          local changed = VT:SetMapSublevel(self.index)
          VT:SetSelectionToPull(selection)
          if changed then VT:UpdateMap() end
        elseif (mouseButton == "RightButton") then
          local maxPulls = #VT:GetCurrentPreset().value.pulls
          if maxPulls > 1 then
            VT:DeletePull(self.index)
          end
        end
      else
        VT:EnsureDBTables()
        if (mouseButton == "RightButton") then
          ...
          --normal click
          VT:GetCurrentPreset().value.selection = { self.index }
          local changed = VT:SetMapSublevel(self.index)
          VT:SetSelectionToPull(self.index)
          if changed then VT:UpdateMap() end
        end
    ...
    function self.callbacks.OnEnter()
      ...
      for _, button in pairs(VT.main_frame.sidePanel.newPullButtons) do
        if button.dragging then return end
      end
      VT.pullTooltip:SetPoint("TOPRIGHT", self.frame, "TOPLEFT", 0, 0)
      VT.pullTooltip:SetPoint("BOTTOMRIGHT", self.frame, "TOPLEFT", -250, -(4 + VT.pullTooltip.myHeight))
      local tooltipBottom = VT.pullTooltip:GetBottom()
      local mainFrameBottom = VT.main_frame:GetBottom()
      if tooltipBottom < mainFrameBottom then
        VT.pullTooltip:SetPoint("TOPRIGHT", self.frame, "BOTTOMLEFT", 0, (4 + VT.pullTooltip.myHeight))
        VT.pullTooltip:SetPoint("BOTTOMRIGHT", self.frame, "BOTTOMLEFT", -250, -4)
      end
      self.entered = true
      VT:ActivatePullTooltip(self.index)
      self.frame:SetScript("OnUpdate", self:CreateUpdateFunction())
      --progressbar
      if VT.ProgressBarResetTimer then VT.ProgressBarResetTimer:Cancel() end
      local currentForces = VT:CountForces(self.index)
      local db = VT:GetDB()
      local teeming = VT:IsCurrentPresetTeeming()
      VT:Progressbar_SetValue(VT.main_frame.sidePanel.ProgressBar, currentForces,
        teeming and VT.dungeonTotalCount[db.currentDungeonIdx].teeming or
        VT.dungeonTotalCount[db.currentDungeonIdx].normal)
      VT:PullClickAreaOnEnter(self.index)
    end

    function self.callbacks.OnLeave()
      VT.pullTooltip.Model:Hide()
      VT.pullTooltip.topString:Hide()
      self.entered = false
      self.frame:SetScript("OnUpdate", nil)
      VT:UpdatePullTooltip(VT.pullTooltip)
      VT.pullTooltip:Hide()
      VT.ProgressBarResetTimer = C_Timer.NewTimer(0.35, function()
    ...
    if not self.updateFunction then
      self.updateFunction = function(frame, elapsed)
        ...
        if self.dragging then
          if VT.pullTooltip:IsShown() then
            VT.pullTooltip:Hide()
          end
          ...
          if scrollFrame.frame:IsMouseOver(scroll_hover.height, height - scroll_hover.offset, -dragdrop_overlap,
            ...
            if self.top_hover > scroll_hover.timeout then
              ...
              local scroll_pulls = VT.U.lerp(scroll_hover.pulls_per_second.min, scroll_hover.pulls_per_second.max,
                scroll_speed)
              local scroll_pixel = scroll_pulls * self.frame:GetHeight()
              local scroll_amount = VT:GetScrollingAmount(scrollFrame, scroll_pixel) * scroll_hover.timeout
          ...
          if self.elapsed > 0.1 then
            ...
            VT:Show_DropIndicator(button, pos)
            self.elapsed = 0
          end
        end
      end
  ...
  ["Drag"] = function(self)
    local sidePanel = VT.main_frame.sidePanel
    ...
    if #VT:GetSelection() > 1 then
      if not VT.U.contains(VT:GetSelection(), self.index) then
        ...
        VT:GetCurrentPreset().value.currentPull = self.index
        VT:GetCurrentPreset().value.selection = { self.index }
        self:Pick()
      end
  ...
  ["Drop"] = function(self)
    ...
    if #VT:GetSelection() > 1 then
      ...
      for offset, pullIdx in ipairs(selected_pulls) do
        ...
        local progressed_above = VT.U.count_if(progressed_pulls, function(entry)
          return entry < pos
        end)
        ...
        if pos > correctPullIndex then
          correctPullIndex = correctPullIndex - VT.U.count_if(progressed_pulls, function(entry)
        ...
        VT:PresetsAddPull(pos)
        VT:CopyPullOptions(correctPullIndex, pos)
        local newID = VT:PresetsMergePulls(correctPullIndex, pos)
        --print("newID", newID)
      ...
      VT:GetCurrentPreset().value.selection = new_pulls
      VT:ReloadPullButtons()
      VT:SetSelectionToPull(1)
    else
      ...
      VT:PresetsAddPull(insertID)
      VT:CopyPullOptions(index, insertID)
      local newIndex = VT:PresetsMergePulls(index, insertID)
      VT:ReloadPullButtons()
      VT:SetSelectionToPull(newIndex)
    end

    VT:Hide_DropIndicator()
    --VT:UpdateAutomaticColors(math.min(self.index, insertID))
    VT.pullTooltip:Show()
    if VT.liveSessionActive and VT:GetCurrentPreset().uid == VT.livePresetUID then
      VT:LiveSession_SendPulls(VT:GetPulls())
    end
  ...
  ["SetIndex"] = function(self, index)
    ...
    self.color.r, self.color.g, self.color.b = VT:DungeonEnemies_GetPullColor(self.index)
    self:UpdateColor()
  end,
  ...
  ["ShowReapingIcon"] = function(self, show, currentForces, oldForces, totalForcesMax)
    ...
    local pullForces = VT:CountForces(self.index, true)
  ...
  ["UpdateColor"] = function(self)
    local colorHex = VT:RGBToHex(self.color.r, self.color.g, self.color.b)
    local db = VT:GetDB()
...
local function Constructor()
  local name = "VTPullButton"..AceGUI:GetNextWidgetNum(Type);
  ...
  pickedGlow:SetTexture("Interface\\AddOns\\VisionTools\\Textures\\HeartOfAzerothSelection")
  ...
  for i = 1, maxPortraitCount do
    ...
    enemyPortraits[i].overlay:SetTexture("Interface\\Addons\\VisionTools\\Textures\\Circle_White")
  ...
  reapingIcon.overlay:SetTexture("Interface\\Addons\\VisionTools\\Textures\\Circle_White")
